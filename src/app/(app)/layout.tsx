...
export default function AuthenticatedAppLayout({
    ...
    return (
      <div className="flex items-center justify-center h-screen bg-background">
        <div className="flex flex-col items-center space-y-4">
          ...
          <p className="text-muted-foreground">{t.financialManagerLoading}</p> {/* Menggunakan string terjemahan */}
        </div>
      </div>
    );
  }
  ...
  if (!currentUser) {
    // Ini adalah fallback jika redirect dari useEffect belum terjadi.
    // Seharusnya pengguna tidak akan melihat ini karena redirect akan terjadi cepat.
    return (
      <div className="flex items-center justify-center h-screen bg-background">
        <div className="flex flex-col items-center space-y-4">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-16 h-16 text-primary animate-pulse">
